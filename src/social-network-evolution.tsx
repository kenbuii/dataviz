...
const SocialNetworkEvolution: React.FC = () => {
  ...
  const strongLinksData: StrongLinksDataPoint[] = days.map(day => {
    const result: StrongLinksDataPoint = { 
      day: `Day ${day}`,
      sim1Right: 0,
      sim2Left: 0,
      sim3Left: 0,
    ...
    // Simulation 1
    const sim1Data = simulationData[0].data.find((d: SimulationDataPoint) => d.day === day);
    result.sim1Right = sim1Data?.strongRight || 0;
    
    // Simulation 2
    const sim2Data = simulationData[1].data.find((d: SimulationDataPoint) => d.day === day);
    result.sim2Left = sim2Data?.strongLeft || 0;
    
    // Simulation 3
    const sim3Data = simulationData[2].data.find((d: SimulationDataPoint) => d.day === day);
    result.sim3Left = sim3Data?.strongLeft || 0;
    result.sim3Right = sim3Data?.strongRight || 0;
    result.sim3Cross = sim3Data?.strongCross || 0;
    
    // Simulation 4
    const sim4Data = simulationData[3].data.find((d: SimulationDataPoint) => d.day === day);
    ...
    <div className="flex flex-col space-y-8">
      ...
      <div className="p-4 bg-gray-50 rounded-lg shadow">
        ...
        <ul className="list-disc pl-6 space-y-2">
          ...
          <li>The mixed networks (20L-30R) generate significantly higher total revenue despite lower read rates.</li>
